...
| 단계 | 검증 방법 | 완료 |
|---|---|---|
| 1. index.html 기본 골격 + Tailwind CDN 연결 | 브라우저에서 페이지 로드 확인 | [ ] |
| 2. 추가 폼 UI (title/due_at/status) | 폼 요소 렌더링 확인 | [ ] |
| 3. 목록 카드 UI (status 배지, 남은 시간) | 더미 데이터로 카드 렌더링 확인 | [ ] |
| 4. 수정 모달 UI (전 필드) | 카드 클릭 시 모달 오픈 확인 | [ ] |
| 5. 삭제 확인 다이얼로그 UI | 휴지통 클릭 시 확인창 노출 확인 | [ ] |
| 6. app.js에서 API 연동 (CRUD 4종) | 실제 백엔드 응답으로 화면 갱신 확인 | [ ] |
| 7. 테마 토글 + 반응형(360px) 적용 | localStorage 유지, 360px에서 레이아웃 확인 | [ ] |
| 8. 전체 동작 확인 후 git add/commit/push | 성공 기준 4가지 통과 후 원격 반영 확인 | [ ] |
